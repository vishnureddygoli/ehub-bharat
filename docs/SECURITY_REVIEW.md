...
| S4 | CSP strength unverified — confirm a restrictive `Content-Security-Policy` (script-src self, no unsafe-inline where feasible), `Strict-Transport-Security`, `Referrer-Policy`, `Permissions-Policy`, `X-Content-Type-Options: nosniff`, `X-Frame-Options`/`frame-ancestors`. | Medium | 7 |
